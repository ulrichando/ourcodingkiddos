...
export default function ChatBot() {
  ...
  return (
    <>
      ...
      {isOpen && (
        <Card className="fixed bottom-6 right-6 z-50 w-96 h-[500px] flex flex-col shadow-2xl rounded-2xl overflow-hidden border-0">
              ...
              <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                <div
                  className={`max-w-[80%] rounded-2xl px-4 py-2 ${
                    msg.role === "user" ? "bg-purple-500 text-white rounded-br-sm" : "bg-white text-slate-700 rounded-bl-sm shadow-sm"
                  }`}
                >
          ...
          <div className="px-4 py-2 flex flex-wrap gap-2 bg-white border-t">
            ...
            <button
              onClick={handleClear}
              className="text-xs px-3 py-1.5 rounded-full bg-slate-100 text-slate-700 hover:bg-slate-200 transition-colors inline-flex items-center gap-1"
            >
