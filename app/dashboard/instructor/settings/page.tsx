  ...
  const loadPreferences = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setNewStudentAlerts(data.newStudentAlerts ?? true);
      }
    } catch (error) {
  ...
  const handleSavePreferences = async () => {
    ...
    try {
      const response = await fetch("/api/preferences", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ emailUpdates, classReminders, newStudentAlerts }),
      });
      if (response.ok) {
    ...
    <InstructorLayout>
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <Card className="border-0 shadow-sm">
          ...
          <CardContent className="space-y-4">
            <p className="text-sm text-slate-600 dark:text-slate-400">
              Configure your availability and teaching settings in the{" "}
              <a href="/dashboard/instructor/availability" className="text-emerald-600 dark:text-emerald-400 hover:underline">
                Availability
              </a>{" "}
              section.
            </p>
        ...
        <Card className="border-0 shadow-sm">
          ...
          <CardContent className="space-y-4">
            {[
              ...
              { id: "classReminders", label: "Class Reminders", desc: "Get notified before your scheduled classes", value: classReminders, setter: setClassReminders },
              { id: "newStudentAlerts", label: "New Student Alerts", desc: "Be notified when a new student enrolls", value: newStudentAlerts, setter: setNewStudentAlerts },
            ].map((pref) => (
              <div key={pref.id} className="flex items-center justify-between">
                ...
                <button
                  ...
                  className={`relative w-11 h-6 rounded-full transition-colors ${pref.value ? "bg-emerald-500" : "bg-slate-300 dark:bg-slate-600"}`}
                >
        ...
        <Card className="border-0 shadow-sm border-red-200 dark:border-red-900">
          ...
          <CardContent className="space-y-4">
            <p className="text-sm text-slate-600 dark:text-slate-400">
              Deleting your account will permanently remove all your data, including teaching records and student associations.
            </p>
